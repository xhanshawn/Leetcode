class Solution:
    ...
    def isMatch(self, s, p):
        ...
        star_i, star_j = -1, -1
        while(i < len(s)):
            if(j < len(p) and (s[i] == p[j] or p[j] == '?')): i, j = i + 1, j + 1
        ...
        return True
                # if(p[j] == '*'):
                #     # p's latter and s's current are same
                #     if(j + 1 < len(p) and s[i] == p[j + 1]): 
                #         while(i + 1 < len(s) and s[i + 1] == s[i]): i += 1
                #         while(j + 1 < len(p) and p[j + 1] == p[i]): j += 1
                #         # i, j = i + 1, j + 1
                #         # only s's latter are same
                ...
                # else: return False
        # print i, len(s)
    # above is Accepted solution very good idea without dp. not created by myself
    ...
    def isMatch2(self, s, p):
        ...
        for i in xrange(len(s)):
            p = i if(not i == 1) else 0
            for j in xrange(len(p)):
                    # dp[]
                # dp[i + 1][j + 1] = dp[i][j] and (s[i] == p[j] or p[j] == '?' or p[j] == '*') 
                # dp[i + 1][j + 1] |= dp[i][j + 1] and (p[j] == '*') 
                if(dp[i + 1][j + 1]): print i + 1, j + 1
                # if(i > 0): dp[i + 1][j + 1] |= dp[i][j + 1] and p[j] == '*' and s[i] == s[i - 1]
                # if(i > 0 and j > 0): dp[i + 1][j + 1] |= (s[i - 1] == s[i]) and (p[j - 1] == '*')
        # print dp
        return dp[-1][-1]
s = Solution()
